...
class Reference {
    ...
    다음 토큰은 항상 키워드로 해석하며 식별자로 사용할 수 없다:
        ...
        콜렉션이나 'contains' 메서드를 정의한 다른 엔티티, 범위 에 값이 속하는지 검사하기 위한 중위 연산자로 사용
        when 식 에서 같은 목적으로 사용
        반공변 으로 타입 파라미터 지정
        !in
        콜렉션이나 'contains' 메서드를 정의한 다른 엔티티, 범위 에 값이 속하지 않는지 검사하기 위한 중위 연산자로 사용
        when 식 에서 같은 목적으로 사용
        interface : 인터페이스
        is
        값이 특정 타입인지 검사
    ...
    operator : 함수를 연사자 오버로딩이나 컨벤션 구현으로 표시
