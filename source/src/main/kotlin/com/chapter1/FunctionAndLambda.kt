...
//    }
    ...
    //인라인 함수
    /*
    고차 함수 를 사용하면 런타임에 일부 불이익이 발생한다. 각 함수는 객체이고 함수의 몸체에서 접근하는 변수인 클로저를 캡처한다. 메모리 할당(함수 객체와 클래스 둘에
    대해)과 가상 호출로 런타임 오버헤드가 증가한다.
    하지만 많은 경우에 람다 식을 인라인해서 이런 종류의 오버헤드를 없앨 수 있다. 아래 함수가 이런 상황의 종은 예다. lock() 함수를 쉽게 호출 위치에 인라인할 수 있
    다. 다음 경우를 보자:
     */
    //lock(l) { foo() }
    ...
    //비-로컬 리턴
    //코틀린에서 이름을 가진 함수나 익명 함수에서 나가려면 일반적인 한정하지 않은 return 을 사용해야 한다. 이는 람다를 나가려면 라벨 을 사용해야 한다는 것을 의미한
    //다. 람다에서의 순수 return 은 금지하는데 람다가 둘러싼 함수를 리턴할 수 없기 때문이다:
//    fun foo() {
//        ordinaryFunction {
...
//    }
    ...
    //코루틴
    //일부 API는 길게 실행되는 오퍼레이션을(네트워크 IO, 파일 IO, CPU나 GPU에 집중된 작업 등) 시작하고, 끝날 때까지 호출자가 멈출 것을 요구한다. 코루틴은 쓰레드 차
    //단을 피하고 더 싸고 더 제어가능한 오퍼레이션으로 대체하는 방법을 제공한다. 그 오퍼레이션은 코루틴의 서스펜션(suspension) 이다.
    //코루틴은 비동기의 복잡함을 라이브러리에 넣어서 비동기 프로그래밍을 단순하게 한다. 코루틴을 사용하면 프로그램 로직을 순차적으로 표현할 수 있는데 하위 라이브러리
    //는 이를 비동기로 해석한다.
    //라이브러리가 사용자 코드의 관련 부분을 콜백으로 감싸고, 관련 이벤트를 구독하고, 다른 쓰레드로 실행 스케줄을 잡지만(심지어 다른 머신에서), 코드는 마치 순차적으로
    //실행하는 것처럼 단순한 형태를 유지한다.
    //다른 언어에서 사용할 수 있는 다양한 비동기 기법을 코틀린의 코루틴을 이용해서 라이브러리로 구현할 수 있다. 이런 라이브러리에는 C#과 ECMAScript에서 온 async
    /// await , Go에서 온 채널 과 select , C#과 파이썬에서 온 제너레이터/ yield 가 있다. 그런 구성 요소를 제공하는 라이브러리에 대한 설명은 아래 를 참고한다.
    //기본적으로 코루틴은 쓰레드 블로킹 없이 연기 할 수 있는 계산이다. 쓰레드 블로킹은 비싸다. 특히 부하가 높은 상황에서는 상대적으로 작은 개수의 쓰레드만 실질적으로 사
    //용되기 때문에 그 쓰레드 중 하나를 블록킹하면 일부 중요한 작업을 지연시키게 된다.
    //반면에 코루틴 서스펜션은 비용이 거의 없다. 컨텍스트 스위치나 OS와 관련된 다른 것이 필요없다. 그 기반하에 사용자 라이브러리는 높은 수준으로 서스펜션을 제어할 수
    //있다. 우리는 라이브러리 작성자로서 우리 요구에 따라 어떤 서스펜션이 발생하고 최적화/로그/가로채기할지 결정할 수 있다.
    //또 다른 차이점은 코루틴은 임의의 명령어를 연기할 수 없다는 것이다. 코루틴은 서스펜션 포인트 라고 불리는 곳만 연기할 수 있다. 서스펜션 포인트는 특별히 표시한 함수
    //를 호출하는 지점이다.
    ...
    //이런 함수를 서스펜딩 함수(suspending function) 라고 부르는데, 이 함수를 호출하면 코루틴을 연기하기 때문이다(라이브러리는 함수 호출에 대한 결과가 이미 사용가능
    //하면, 서스펜션 없이 진행할지 여부를 결정할 수 있다). 서스펜딩 함수는 일반 함수와 같은 방법으로 파라미터와 리턴 값을 가질 수 있다. 하지만 서스펜딩 함수는 코루틴이
    //나 다른 서스펜딩 함수에서만 호출할 수 있다. 사실 코루틴을 시작하려면, 적어도 한 개의 서스펜딩 함수가 있어야 하며 보통 익명 함수를 사용한다(즉, 서스펜딩 람다). 예제
    //를 살펴보자. 다음은 단순화한 async() 함수이다( kotlinx.coroutines 라이브러리).

    //fun <T> async(block: suspend () -> T)



    //여기서 async() 는 일반 함수인데(서스펜딩 함수가 아님), block 파라미터는 suspend 수식어를 가진 suspend () -> T 함수 타입이다. 람다를
    //async() 에 전달하면, 그것은 서스펜딩 람다 가 되고, 그 람다에서 서스펜딩 함수를 호출할 수 있다:
    ...
    //공통점을 계속 보면, await() 는 서스펜딩 함수일 수 있다(그래서 aync {} 블록 안에서 호출가능함), 이는 특정 계산이 끝나고 그 결과를 리턴할 때까지 코루틴을
    //연기한다:
